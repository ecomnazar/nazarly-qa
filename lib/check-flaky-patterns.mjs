...
// incl. cy.wait(N) and 'networkidle' waits, shared /tmp paths, wall-clock
// assertions, a forgotten .only, shared mock state, unseeded randomness).
//
...
const PATTERNS = [
  ...
  ['sleep-as-sync', /\bcy\.wait\s*\(\s*\d/, "cy.wait(<ms>) is a fixed sleep — Cypress calls it an anti-pattern; wait on an intercept alias cy.wait('@route') or assert the expected state (Cypress auto-retries). cy.wait('@alias') is fine and not flagged"],
  ['networkidle', /['"`]networkidle[02]?['"`]/, "'networkidle' wait (incl. Puppeteer's networkidle0/2) — Playwright discourages it for tests: one poll/analytics/websocket keeps the network busy and the wait flakes; wait for a web-first assertion instead. Legit only to assert no background traffic → allowlist it"],
